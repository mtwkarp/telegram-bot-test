...
//refactor
    async getTomorrowTripInstructorsByBase(dayName: DayNames): Promise<Record<string, Array<{ name: string, chatId: string }>>> {
        ...
        const instructorsByBase = await this.getSheetValues({
            range: `${this.sheetCollection.getSheetName('intstructors_by_bases_trip_schedule')}!${sheetLetters.blood}3:${sheetLetters.evacuation}100`
        });
